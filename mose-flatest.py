...
class RWKV6(nn.Module):
    ...
    def load_state(self,state_filename):
        ...
        self.model_current_statetuned.append(model_current_statetuned)
...
if __name__ == '__main__':
    ...
    for i in range(model.n_layer):
        wkv_states[i][0] = model.model_current_statetuned[0][i*3 + 1]
        wkv_states[i][1] = model.model_current_statetuned[1][i*3 + 1]
        wkv_states[i][2] = model.model_current_statetuned[2][i*3 + 1]
    ...
    def calculate_perplexity(logits, target, ignore_index=-100):
        ...
        batch_loss = (loss * mask).sum(dim=1)
        
        avg_loss = batch_loss / num_tokens
        
        perplexity = torch.exp(avg_loss)
        
        return perplexity
    
    perplexities = calculate_perplexity(x, idx, 0)

    for i, perplexity in enumerate(perplexities):
        print(f"Batch {i} Perplexity: {perplexity.item():.4f}")
    ...
    for i in range(1000):
        ...
        #    x[j][-1][0] -= 1e10
        #    token = pipeline.sample_logits_mose2(x[j][-1], temperature=1.5, top_p=0.9)
    ...
    #for token in pipeline.encode(context):
    #    idx = torch.tensor(token)
    #    x, shift_states, wkv_states = model.forward(idx.to('cuda'), shift_states, wkv_states)
    #