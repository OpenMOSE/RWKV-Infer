...
if __name__ == '__main__':
   ...
   #exit()
    ...
    model = RWKV_x('/home/client/Projects/RWKV-LM-RLHF/main/myfolder/models/ARWKV-7B-CJE-30%.pth','fp6',
                   adapter_model='',
                   adapter_mode='bone',
    ...
    messages = [
        ...
        {'role':'user', 'content':'今日は、どうもありがとうございました'}
    ]
    ...
    state_tuned_wkv, offset_tensor = model.load_state('/home/client/Projects/NTTProject/arwkv_nsha_state/rwkv-26-state.pth',EnableOffset=True)
    ...
    print(f'offset_tensor {offset_tensor}')
    offset_tensor = offset_tensor.unsqueeze(0).to('cuda')
    ...
    x1, shift_states1, wkv_states1 = model.forward(copy.deepcopy(idx), copy.deepcopy(shift_states), copy.deepcopy(wkv_states),KernelMode=1,offset_tensor=offset_tensor) #FLA
    del x1
    del shift_states1
    del wkv_states1
    ...
    for i in range(maxtoken):
        ...
        x, shift_states, wkv_states = model.forward(idx, shift_states, wkv_states,one_mode=True,offset_tensor=offset_tensor)
        if x.dim() == 2:
            x = x.view(x.shape[0],1,x.shape[1])
        #print(x)
