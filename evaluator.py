...
class EvaluationConfig:
    ...
    log_path: str = "./uncheatable_eval/logs/"  # path to save the evaluation results
    cache: str = "./uncheatable_eval/models/temp/"  # cache directory for the models
...
class Evaluator:
    ...
    def load_hxa079(self, config: EvaluationConfig):
        # os.environ["RWKV_JIT_ON"] = "1"
        # os.environ["RWKV_CUDA_ON"] = "1"
        # os.environ["RWKV_V7_ON"] = "1"
        ...
        # else:
            #rwkv_model = RWKV(model=config.model_name_or_path.replace(".pth", ""), strategy="cuda fp16")
        rwkv_model = RWKV_x(config.model_name_or_path,'int8')
        #rwkv_pipeline = PIPELINE(rwkv_model, config.tokenizer_name)
        hf_tokenizer = AutoTokenizer.from_pretrained(config.tokenizer_name, cache_dir=config.cache, **config.tokenizer_args)

        #self.print_rwkv7_parameters_in_billions(rwkv_model)

        return rwkv_model, hf_tokenizer
    ...
    def eval_hxa079(self, model, tokenizer, texts, chunk_size):
        rwkv_test_data = []
        rwkv_token_length_list = []
        char_count = []
        ...
        for idx, sample in tqdm(enumerate(texts), total=len(texts)):
            ...
            with torch.no_grad():
                ...
                for begin in range(0, input_length, chunk_size):
                    ...
                    States = model.new_state(1,input_length)
                    ...
                    prompts = []
                    for i in range(1):
                                prompts.append(torch.tensor(input_chunk).unsqueeze(0).to('cuda'))
                    ...
                    #logit = model.forward(input_chunk, None, full_output=True)[0]
                    logit, shift_states, wkv_states,kv_caches,pos_caches = model.forward(copy.deepcopy(idx), shift_states, wkv_states, kv_caches,pos_caches, KernelMode=1,full_output=True)
    ...
    def evaluate(self, config: EvaluationConfig):
        ...
        elif config.model_type == "hxa079":
            model, tokenizer = self.load_hxa079(config)
        else:
        ...
        for data_file in config.data:
            ...
            elif config.model_type in ["hxa079"]:
                results = self.eval_hxa079(model=model, tokenizer=tokenizer, texts=texts, chunk_size=config.chunk_size)
            else:
                raise NotImplementedError
