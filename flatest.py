...
class BlockStateList:
    ...
    def empty(N, B, C, H, device, dtype):
        wkv_states = torch.empty((N, B, H, C//H, C//H),
                                 device=device,
                                 dtype=torch.float)
        shift_states = torch.empty((N*2, 1, B, C), device=device, dtype=dtype)
...
def state_empty(N, B, C, H, device='cuda', dtype=torch.bfloat16):
        wkv_states = torch.empty((N, B, H, C//H, C//H),
                                 device=device,
                                 dtype=torch.bfloat16)
        shift_states = torch.empty((N, 2, B, C), device=device, dtype=dtype)
        #shift_states = shift_states.transpose(0, 1)
...
if __name__ == '__main__':
    ...
    pipeline = PIPELINE()
    model = RWKV6('models/RWKV-x060-World-3B-v2.1-20240417-ctx4096.pth')

    States = model.new_state(1)#state_empty(32, 1, 2560, 2560 // 32)

    context = 'User: What is Large Language Model? \n\nAssistant:'
    ...
    for i in range(100):
        #x[0][0] -= 1e10
        if FirstTime:
            token = pipeline.sample_logits(x[0][0], temperature=1, top_p=0.3)
        else:
            token = pipeline.sample_logits(x[0], temperature=1, top_p=0.3)
