...
model_path = "models/rwkv-x070-2b9-world-v3-preview-20250210-ctx4k.pth"
model_adapter_path = "" # can realtime merge LoRA,Bone,DoRA
model_adapter_mode = "" # set lora,bone,dora
quant_mode = "bf16" # int8, OpenMOSE Silly 8bit matmul kernel(triton)
template = "world"
...
def passkey_retrieval_test(model, tokenizer, device, context_length, depth, seed=666):
    ...
    max_new_tokens = len(answer_ids) + 64
    ...
    with torch.no_grad():
        ...
        for i in range(0, prefill_len, chunk_size):
            ...
            outputs, shift_states, wkv_states = model.forward(chunk, shift_states, wkv_states,KernelMode=1,full_output=True)
...
def main(args):
    ...
    model = RWKV_x(model_path,quant_mode,
                   ...
                   fully_fusedrecurrent=True) #use Fused Kernel(much faster)
