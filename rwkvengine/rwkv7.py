...
class RWKV_7(nn.Module):
    ...
    def x070_TimeMix_fla_Step2(r, w, k, v, aa, bb,state,FullyFusedMode = True,offset_tensor:torch.Tensor=None):
        ...
        if offset_tensor is not None:
            #print(f'offset_tensor state shape = {offset_tensor.shape}')

            r_ = r_.view(B,T,-1)
            offset_tensor = offset_tensor.squeeze(0)
            xx = xx + (offset_tensor @ r.view(H, C, -1)).view(xx.shape)




            # r_reshaped = r.view(B, T, H, C).permute(0, 2, 3, 1)
            # result = offset_tensor @ r_reshaped#torch.matmul(offset_tensor, r_reshaped)
            # result_reshaped = result.permute(0, 3, 1, 2).reshape(B, T, C * H)
            # #print(f'xx = {xx.shape} result_reshaped = {result_reshaped.shape}')
            # xx = xx.view(B, T, C * H) + result_reshaped
        return xx, state
    @MyStatic
        ...
        with torch.no_grad(): 
            ...
            for i in range(self.n_layer):
                ...
                if StrategyMode == 0:
                    ...
                    xx_step2, time_mix_state = RWKV_7.x070_TimeMix_fla_Step2(r,w,k,v,aa,bb,time_mix_state,self.fully_fusedrecurrent,offset_tensor=time_offset_state[i])
